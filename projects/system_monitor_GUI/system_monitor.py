...
if mode == 'one':
    app.layout = html.Div([
        ...
        # New Divs to display current usage values
        html.Div(id='current-ram', style={'fontSize': 20, 'margin': '10px'}),
        html.Div(id='current-cpu', style={'fontSize': 20, 'margin': '10px'}),
        html.Div(id='current-disk', style={'fontSize': 20, 'margin': '10px'}),
    ...
    @app.callback(
        ...
        Output('current-ram', 'children'),
        Output('current-cpu', 'children'),
        Output('current-disk', 'children')],
    ...
    def update_combined_graph(n):
        ...
        # Update Divs with latest values
        ram_text = f"RAM Usage: {data['RAM Usage (%)']}%"
        cpu_text = f"CPU Usage: {data['CPU Usage (%)']}%"
        disk_text = f"Disk Usage: {data['Disk Usage (%)']}%"

        return combined_figure, ram_text, cpu_text, disk_text
